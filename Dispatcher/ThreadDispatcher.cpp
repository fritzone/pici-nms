...
// FS_COMM_1: //first step: receive the Message itselfs
        {
            ...
            if ( iBytes == RECVERROR )
            {
                throw EXCEPTION ( ExceptionDispatcherLostConnection );
            }
            DEBUGSTR ( sMessage );
...
// HANDLE_MSG:
        {
            if ( !getto ( sMessage, sCommand, "#" ) )
            {
                throw EXCEPTION ( ExceptionDispatcherBadResponse );
            }
        }

        if ( sCommand == COMMAND_HELLO )
        {
            DO_CMDHELLO ( sock, sMessage );
        }
        else if ( sCommand == COMMAND_SUBSCRIBE )
        {
            DO_CMDSUBSCRIBE ( sock, sMessage );
        }
        else if ( sCommand == COMMAND_UNSUBSCRIBE )
        {
            DO_CMDUNSUBSCRIBE ( sock, sMessage );
        }
        else if ( sCommand == COMMAND_MESSAGE )
        {
            DO_CMDMESSAGE ( sock, sMessage );
        }
...
void DO_CMDHELLO ( TCPClient* sock, std::string& sMessage ) //CMD HELLO: HELLO#clientNumber@IP:PORT
{
    ...
    if ( !extractto ( sMessage, sCommand, "#" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !extractto ( sMessage, sClient, "@" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !strtoi ( sClient, iClient ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );    // NOT A NUMBER
    }
    if ( !extractto ( sMessage, sIP, ":" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    ASSIGN ( sPort, sMessage );
    if ( !strtoi ( sPort, iPort ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );    // NOT A NUMBER
    }
...
void DO_CMDMESSAGE ( TCPClient* sock, std::string& sMessage ) //7:HELLO.B:ASYNCH:<Message><Parameters></Parameters></Message>
{
    ...
    if ( !extractto ( sTemp, sCommand, "#" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !extractto ( sTemp, sClient, ":" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !extractto ( sTemp, sSubject, ":" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !extractto ( sTemp, sMsgT, ":" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    if ( !extractto ( sTemp, sTTL, ":" ) )
    {
        throw EXCEPTION ( ExceptionDispatcherBadResponse );
    }
    LOG ( "TTL:%s\n", sTTL.c_str() );
    ...
    // TODO: by checking the response from a few lines above we need to create a structure
    // which will hold the clients to which the message was not sent
...
void DO_CMDSUBSCRIBE ( TCPClient* sock, std::string& sMessage ) //CMD SUBSCRIBE : HELLO#sClient:ACK
{
    ...
    {
        throw EXCEPTION ( ExceptionDispatcherNoClient );
    }
    ClientInfo* pClient = ( *itClient ).second;
    SubscriptionInfo* sub = new SubscriptionInfo ( sSubject, pClient );
    subscriptions.push_back ( sub );
    if ( ( iBytes = sock->SendBytes ( ACKM ( iClient, COMMAND_SUBSCRIBE ) ) ) == SENDERROR )
    {
        throw EXCEPTION ( ExceptionDispatcherLostConnection );
    }
...
void DO_CMDUNSUBSCRIBE ( TCPClient* sock, std::string& sMessage ) //CMD UNSUBSCRIBE
{
    ...
    if ( ( iBytes = sock->SendBytes ( ACKM ( iClient, COMMAND_UNSUBSCRIBE ) ) ) == SENDERROR )
    {
        throw EXCEPTION ( ExceptionDispatcherLostConnection );
    }
