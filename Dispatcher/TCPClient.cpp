...
int TCPClient::SendBytes( std::string const& s )
{
	...
	int ret = boost::asio::write( *_me, boost::asio::buffer(C_DATA(s), C_LEN(s)));
	LOG("TCPClient::SendBytes: ret(%d)\n",ret);
	if ( ret < C_LEN(s) ) return SENDERROR;
	return ret;	
}
...
int TCPClient::ReceiveLine( std::string& retStr )
{
	...
	if ( error == boost::asio::error::eof ) return  retBytes; //Connection closed cleanly by peer.
	else if (error) return RECVERROR; // Some other error.
}
