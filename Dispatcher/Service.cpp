 ...
 * Servie constructor
 */
Service::Service( const char* serviceName, ConfigReader& cfg ) :  ServiceName( serviceName?serviceName:"_anonymous" ), pingCounter(0)
{
	LOG("Service::Service()\n");
	me = this;

	// set the settings of the win32 service
#ifdef WIN32	
...
void Service::Stop()
{
	...
	void WINAPI Service::Main( DWORD, LPTSTR* )
	{
		...
		if ( !me->ServiceStatusHandle ) me->LogWindowsEvent( EVENTLOG_ERROR_TYPE,  "Could not register service control handler" );
