...
CI_RESPONSE ClientInfo::sendMessage( std::string msg )
{
	...
	TRY
	{
		...
		FS_SERV1: //sending the message
		{
			...
			if ( bytes == SENDERROR ) throw EXCEPTION ( ExceptionCILostConnection );
		}
		...
		{	
			...
			if ( bytes == RECVERROR ) throw EXCEPTION ( ExceptionCILostConnection );
			if ( !strtoi(ctlResponse,iACK) ) throw EXCEPTION ( ExceptionCIBadResponse );
			else if ( iACK == NOT_ENOUGH_MEMORY ) throw EXCEPTION ( ExceptionCINoMemRemote );
			else if ( iACK != -3 ) throw EXCEPTION ( ExceptionCIBadResponse );			
		}
...
CI_RESPONSE ClientInfo::ping() //PINGS THE CLIENT
{
	TRY
	{
		...
		if ( bytes == CONNECTERROR ) throw EXCEPTION ( ExceptionCICantConnect );
		bytes = client.SendBytes(PING);
		if ( bytes == SENDERROR ) throw EXCEPTION ( ExceptionCILostConnection );
		bytes = client.ReceiveBytes(ctlResponse);
		if ( bytes == RECVERROR ) throw EXCEPTION ( ExceptionCILostConnection );
		if ( ctlResponse == "PONG") return CI_OK;
