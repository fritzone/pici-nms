...
TransporterImpl::~TransporterImpl(void)
{
	...
	if(NULL != initializer) delete initializer;
	if(NULL != dispatcherAddress) delete dispatcherAddress;
	if(NULL != identity) delete identity;
	if(NULL != localTcpServer) delete localTcpServer;
	if(NULL != tcpcf) delete tcpcf;
	if(NULL != ca) delete ca;
}
