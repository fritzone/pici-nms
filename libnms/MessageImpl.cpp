...
double MessageImpl::getDouble ( const string& doubleName )
{
    for ( size_t i = 0; i < parameters.size(); i++ )
    {
        if ( ObjImplMapper<Parameter, ParameterImpl>::getImpl ( parameters[i] )->getName() == doubleName )
        {
            ...
            if ( doubleI )
            {
                return doubleI->operator double();
            }
        }
    }
    return 0;
